...
function createDomainVisual() {
    ...
    let nodes
    let edges
    let edges_country, edges_domain = []
    ...
    const inner_radius_element = radius_elements - 2*node_radius - 1.75*node_radius
    const outer_radius_element = radius_elements + 2*node_radius + 1.75*node_radius
    ...
    const scale_angle_start_offset = d3.scaleLinear()
        .domain([0, 1])
        .range([0, 0.03])
    const scale_angle_end_offset = d3.scaleLinear()
        .domain([0, 1])
        .range([0, 0.02])
    ...
    const color_domain_scale = d3.scaleOrdinal()
    ...
    const scale_font_size = d3.scaleLinear()
        .domain([100, 300, 500])
        .range([30, 26, 24])
    ...
    function dataPreparation() {
        ...
        element_ids.forEach(d => {
            ...
            node.domains.sort()
            node.domain_code = node.domains.map(n => n.replace("vocabulary_ich_", "")).join("_")

            //The main domain of an element - only one has a weight of 3
            let domain_edges = edges.filter(l => l.source === d && node.domains.indexOf(l.target) >= 0)
            ...
            node.domains.forEach(m => { domain_by_id[m].count += 1 })
        ...
        country_ids.forEach(d => {
            ...
            let elements = linked_to_id[d].filter(n => n.type === "element").map(n => n.id)
        ...
        edges_country = edges.filter(d => {
            if (element_ids.indexOf(d.source) >= 0 && country_ids.indexOf(d.target) >= 0) return true
            else if (element_ids.indexOf(d.target) >= 0 && country_ids.indexOf(d.source) >= 0) return true
            else return false
        })//filter
    ...
    function prepareArcData() {
        ...
        domain_arcs = pie_domain.padAngle(.02)(domains)
        ...
        element_arcs.forEach(d => {
            ...
            d.data.values.forEach((n,i) => {
                if(num === 1) { //Place in the center if there's only 1 option
                    n.angle = d.centerAngle
                    n.x = radius_elements * Math.cos(n.angle - pi1_2)
                    n.y = radius_elements * Math.sin(n.angle - pi1_2)
                } else if(num === 2) {
                    n.angle = d.centerAngle
                    if(i%3 === 0) radius = radius_elements + radius_elements_offset*2/3
                    ...
                    else if(i%3 === 1) radius = radius_elements + radius_elements_offset
    ...
    function prepareEdgeData() {
        ...
        edges_domain_nest = d3.nest()
            .key(d => d.source.data.id)
            .key(d => d.rotation)
            .entries(edges_domain)
        edges_domain_nest.forEach(d => {
            d.values.forEach(l => { l.edges_count = l.values.length })
        })//forEach
        ...
        edges_element_domain_nest = d3.nest()
        ...
        edges_element_country_nest = d3.nest()
            .key(d => d.source.id)
            .key(d => d.rotation)
    ...
    ////////////////////// Draw curved text //////////////////////
    function drawTextAlongArc(ctx, str, angle, radius, side, kerning = 0.6, do_stroke = false){
        ...
        for (let n = 0; n < str.length; n++) {
            ...
            if(do_stroke) ctx.strokeText(str[n], 0, (side === "up" ? -1 : 1) * radius)
    ...
    function drawHiddenElements() {
        ...
        hover_country = g_scale.selectAll(".country-hover-path")
            .data(countries)
            .enter().append("path")
    ...
    function prepareArcs() {
        ...
        arc_country
            ...
            .outerRadius(d => outer_radius_country + 2*country_radius + d.width + 10)
    }//function prepareArcs
    ...
    function drawDomainDonutChart(ctx, d, opacity) {
        ...
        //Draw the arc
        ctx.fillStyle = chroma(color_domain_scale(d.data.id)).alpha(opacity).css()
        ctx.beginPath()
        ...
        let side = (d.centerAngle < pi1_2 || d.centerAngle > pi2 - pi1_2 ? "up" : "down")
        let radius = inner_radius_domain + (outer_radius_domain - inner_radius_domain)/2
    ...
    function drawElementDonutChart() {
        element_arcs.forEach(d => {
            ctx_donuts.fillStyle = chroma("rgb(230,230,230)").alpha(d.opacity).css()
    ...
    function drawCountryDonutChart() {
        ...
        area_arcs.forEach(d => {
            ...
            ctx_donuts.fillStyle = chroma(color_country).alpha(d.opacity).css()
            ctx_donuts.beginPath()
            arc_area(d)
            ctx_donuts.closePath()
            ctx_donuts.fill()
            ...
            ctx_donuts.strokeStyle = chroma("white").alpha(d.opacity_text).css()
            ctx_donuts.fillStyle = chroma(color_country).alpha(d.opacity_text).css()
            ...
            drawTextAlongArc(ctx_donuts, d.data.key, angle, radius, side, kerning = 1.4, do_stroke = true)
        })//forEach
    ...
    function drawElements(ctx, d, opacity) {
        ...
        arcs.forEach(a => {
            ...
            ctx.closePath()
            ctx.fillStyle = chroma(color_domain_scale(a.data)).alpha(opacity).css()
            ctx.fill()
        })//forEach
        ctx.restore()

        //Draw a stroke around the circles with a main domain
        if(d.domain_main !== "0") { //} && opacity === 1) {
            ctx.globalCompositeOperation = "source-atop"
            ctx.beginPath()
            // ctx.moveTo(d.x, d.y)
            ctx.arc(d.x, d.y, node_radius - element_stroke_width*0.4, 0, pi2)
            ctx.closePath()
            ctx.strokeStyle = chroma(color_domain_scale(d.domain_main)).alpha(opacity).css()
            ctx.stroke()
    ...
    function drawCountries(ctx, d, opacity) {
        ...
        ctx.closePath()
        ctx.fillStyle = chroma(color_country).alpha(opacity).css()
        ...
        ctx.fillStyle = chroma(color_text).alpha(opacity_text).css()
    ...
    function drawDomainDots() {
        ...
        domain_arcs.forEach(d => {
            ctx_nodes.fillStyle = chroma(color_domain_scale(d.data.id)).alpha(d.opacity).css()
    ...
    function drawElementArcDots() {
        ...
        element_arcs.forEach(d => {
            ...
            ctx_nodes.fillStyle = chroma(chosen_domain_color).alpha(op).css()
            ctx_nodes.strokeStyle = chroma("white").alpha(op).css()
            ctx_nodes.beginPath()
    ...
    function drawLines(ctx, type, data, filter_id) {
        data.forEach(e => { //outer domains / elements / countries
            e.values.forEach(c => { //ccw vs cw
                c.values.forEach((d,i) => { //edges
                    if(d.opacity === 0) return

                    if(type === "domain" && filter_id && d.source.data.id !== filter_id) return
                    else if(type === "element-domain" && filter_id && filter_id.indexOf(d.target.data.key) < 0) return
                    else if(type === "element-country" && filter_id && d.source.id !== filter_id) return
                    else if(type === "country" && filter_id && d.target.id !== filter_id) return
                    ...
                    let start_angle = source_a + d.angle_sign * scale_angle_start_offset(d.total_angle) * pi
                    //Slightly offset the last point on the curve from the target
                    let end_angle = target_a - d.angle_sign * scale_angle_end_offset(d.total_angle) * pi

                    let da_inner
                    if (target_a - source_a < -pi) da_inner = pi2 + (end_angle - start_angle)
                    else if (target_a - source_a < 0) da_inner = (start_angle - end_angle)
                    else if (target_a - source_a < pi) da_inner = (end_angle - start_angle)
                    else if (target_a - source_a < pi2) da_inner = pi2 - (end_angle - start_angle)
                    ...
                    ctx.beginPath()
                    line(line_data)
                    let stroke = (type === "domain" || type === "element-domain" ? color_domain_scale(d.source.data.id) : color_country)
                    ctx.strokeStyle = chroma(stroke).alpha(d.opacity).css()
                    ctx.stroke()
                })//forEach d
            })//forEach c
        })//forEach e
    ...
    function drawInnerSection(ctx, d) {
        ...
        //Draw the circles as mini pie charts
        let arcs = (d.domain_main !== "0" ? pie_nodes([d.domain_main]) : pie_nodes(d.domains))
        arcs.forEach(a => {
            ctx.beginPath()
        ...
        ctx.fillStyle = (d.domain_main !== "0" ? color_domain_scale(d.domain_main) : (d.domains.length === 1 ? color_domain_scale(d.domains[0]) : "black"))
    ...
    function drawCanvas() {
        ...
        if(hover_type === "element") drawLines(ctx_edges, "element-domain", edges_element_domain_nest)
        else if(hover_type !== "country") drawLines(ctx_edges, "domain", edges_domain_nest)
        if(hover_type === "element") drawLines(ctx_edges, "element-country", edges_element_country_nest)
        ...
        ctx_donuts.textBaseline = "middle"
        ctx_donuts.textAlign = "center"
        domain_arcs.forEach(d => { drawDomainDonutChart(ctx_donuts, d) })
        ...
        countries.forEach(d => { drawCountries(ctx_nodes, d) })

        //Draw the title in the center
        if(hover_type === "element") drawInnerSection(ctx_nodes, current_hover)
        else if(hover_type === "domain") drawDomainTitle(current_hover.data, ICH_num)
        else if(hover_type === "country") drawCountryTitle(current_hover.label, ICH_num)
    ...
    function mouseOverDomain(d) {
        ...
        //The edges
        edges_domain.forEach(l => { l.opacity = (l.source.data.id === id ? 1 : 0) })

        //The domain arcs
        domain_arcs.forEach(n => { n.opacity = (n.data.id === id ? 1 : 0.1) })

        //The ICH element arcs
        element_arcs.forEach(n => { n.opacity = (n.data.key.includes(id_num) ? 1 : 0.1) })
        //The dots outside the ICH element arcs
        chosen_domain_color = color_domain_scale(id)

        //The ICH circles
        elements.forEach(n => { n.opacity = (n.domains.indexOf(id) >= 0 ? 1 : 0.1) })
        ...
        //Connected elements 
        let connected_elements = elements.filter(n => n.opacity >= 0.99).map(n => n.id)
        countries.forEach(n => {
    ...
    function mouseOverElement(d) {
        ...
        edges_domain.forEach(l => { l.opacity = (l.target.data.key === domain_id ? 1 : 0) })

        //The domain arcs
        domain_arcs.forEach(n => { n.opacity = (d.domains.indexOf(n.data.id) >= 0 ? 1 : 0.2) })

        //The ICH element arcs
        element_arcs.forEach(n => { n.opacity = (n.data.key === domain_id ? 0.7 : 0.1) })
        ...
        edges_country.forEach(l => { l.opacity = (l.source.id === id ? 1 : 0) })
    ...
    function mouseOverCountry(d) {
        ...
        //The edges from element to domain
        edges_domain.forEach(l => { l.opacity = 0 })

        //The domain arcs
        domain_arcs.forEach(n => { n.opacity = (domains.indexOf(n.data.id) >= 0 ? 1 : 0.2) })

        //The ICH element arcs
        element_arcs.forEach(n => { n.opacity = (domain_codes.indexOf(n.data.key) >= 0 ? 0.5 : 0.1) })

        //The ICH circles
        elements.forEach(n => { n.opacity = (d.elements.indexOf(n.id) >= 0 ? 1 : 0.1) })
        ...
        edges_country.forEach(l => { l.opacity = (l.target.id === id ? 1 : 0) })
    ...
    function fadeCanvasBackIn() {
        ...
        edges_domain.forEach(n => { n.interpolate_opacity = d3.interpolate(n.opacity, opacity_edge_default) })
        //Domain arcs
        domain_arcs.forEach(n => { n.interpolate_opacity = d3.interpolate(n.opacity, 1) })
        //The ICH element arcs
        element_arcs.forEach(n => { n.interpolate_opacity = d3.interpolate(n.opacity, 1) })
        //The ICH circles
        elements.forEach(n => { n.interpolate_opacity = d3.interpolate(n.opacity, 1) })
        //The edges between the ICH and countries
        edges_country.forEach(n => { n.interpolate_opacity = d3.interpolate(n.opacity, 0) })
        //The country arcs
        area_arcs.forEach(n => { 
        ...
        countries.forEach(n => { 
            ...
            n.interpolate_opacity_text = d3.interpolate(n.opacity_text, country_text_opacity)
        })
        ...
        timer_draw = d3.timer(elapsed => {
            ...
            //Set new opacities
            elements.forEach(n => { n.opacity = n.interpolate_opacity(t) })
            countries.forEach(n => { 
                n.opacity = n.interpolate_opacity(t)
                n.opacity_text = n.interpolate_opacity_text(t)
            })
            edges_domain.forEach(l => { l.opacity = l.interpolate_opacity(t) })
            edges_country.forEach(l => { l.opacity = l.interpolate_opacity(t) })
            domain_arcs.forEach(n => { n.opacity = n.interpolate_opacity(t) })
            element_arcs.forEach(n => { n.opacity = n.interpolate_opacity(t) })
            area_arcs.forEach(n => { 
                n.opacity = n.interpolate_opacity(t)
                n.opacity_text = n.interpolate_opacity_text(t)
            })
    ...
    chart.saveImage = (width_print = 20, units = "cm") => {
        ...
        function createPrintCanvas() {
            ...
            ctx_save.drawImage(canvas_donuts.node(), 0, 0, canvas_save.width, canvas_save.height)
