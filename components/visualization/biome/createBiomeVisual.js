...
export default function createBiomeVisual() {
  ...
  const biome_metadata = [
    ...
    {
      id: 'vocabulary_ich_930',
      color: 'rgb(160,160,160)',
      file: 'non-ecosystem.svg',
  ...
  //Node pie charts
  const arc_elements = d3.arc().innerRadius(0);
  const pie_elements = d3
    .pie()
    .sort(null)
    .value(1);
  ...
  const degree_scale = d3
    .scaleLinear() //linear on purpose
    .domain([0, 50])
    .range([4, 45]);

  //Basic color that is given to the nodes based on their type
  const color_type_scale = d3
    .scaleOrdinal()
    .domain(types)
  ...
  const edge_distance_scale = d3
    .scaleOrdinal()
    .domain([3, 2, 1])
    .range([30, 100, 120])
    .unknown(5);
  //Simulation scale | How strong do the edges play a role in node positioning based on edge weight
  const edge_strength_scale = d3
    .scaleOrdinal()
    .domain([3, 2, 1])
    .range([0.42, 0.2, 0.1])
    .unknown(0.1);
  //Simulation scale | How long do the edges between nodes "want" to become based on their edge weight
  const edge_distance_secondary_scale = d3
    .scaleOrdinal()
    .domain([3, 2, 1])
    .range([40, 60, 120])
    .unknown(30);
  //Simulation scale | How strong do the edges play a role in node positioning based on edge weight
  const edge_strength_secondary_scale = d3
    .scaleOrdinal()
    .domain([3, 2, 1])
    .range([0.4, 0.1, 0.05])
    .unknown(0.1);
  ...
  function chart(selection, nodes_raw, edges_raw) {
    ...
    modal_icon_circle = node_modal_group
      .append('circle')
    ...
    modal_icon_cross = node_modal_group
      .append('path')
    ...
    simulation_secondary = d3
      ...
      .force(
        ...
        d3
          ...
          .id(d => d.id)
          .distance(d =>
            testTypeId(d, 'whc') ? 10 : edge_distance_secondary_scale(d.weight)
          )
          .strength(d =>
            testTypeId(d, 'whc') ? 1 : edge_strength_secondary_scale(d.weight)
          )
      )
  ...
  function getBiomes() {
    ...
    biomes.forEach(d => {
      ...
      d3.xml('/static/icons/' + d.meta.file).then(xml => {
        let imported_svg = new XMLSerializer().serializeToString(
          xml.documentElement
        );
        let svg_recolor = imported_svg
          .replace('#000000', d.meta.color)
          .replace(/#/g, '%23');
    ...
    biomes = biomes.sort(
      (a, b) => biome_ids.indexOf(a.id) - biome_ids.indexOf(b.id)
    );
    // console.log(biomes)
  } //function getBiomes
  ...
  function filterUnconnectedNodes() {
    ...
    let connected_ids = nodes
      ...
      .map(d =>
        linked_to_id[d.id]
          .filter(n => n.type === 'country' || n.type === 'whc')
          .map(n => n.id)
      );
    ...
    nodes = nodes.filter(d => {
      let degree_now = edges.filter(l => l.source == d.id || l.target == d.id)
        .length;
  ...
  function calculateNodeSettings() {
    ...
    nodes.forEach(d => {
      ...
      let biomes_connected = linked_to_id[d.id].filter(
        n => n.group === 'biome'
      );
      if (biomes_connected.length > 0) {
        ...
        d.biomes.forEach(b => {
          biome_by_id[b].count += 1;
        });
        d.biomes.sort(
          (a, b) =>
            biome_ids.indexOf(biome_by_id[a].id) -
            biome_ids.indexOf(biome_by_id[b].id)
        );
        ...
        if (!d.biome_main) {
          ...
          if (d.biomes.length === 1) {
            ...
            let biome_edge = edges.filter(
              l => l.source === d.id && l.target == d.biome_main
            )[0];
            biome_edge.weight = 3;
          } else {
            //The main biome of an element - only one (should) has a weight of 3
            let biome_edges = edges.filter(
              l => l.source === d.id && d.biomes.indexOf(l.target) >= 0
            );
            ...
            if (biome_main.length === 1) d.biome_main = biome_main[0].target;
              ...
              if (d.type === 'concept')
                biome_edges.forEach(b => (b.weight = 3));
              else console.log('No main biome for', d.id, d.label, d.biomes);
            } else if (biome_main.length > 1)
              console.log('Too many weight 3 biomes for ', d.id, d.biomes);
          } //else
        } //if
      } //if
      ...
      if (d.type === 'concept' && d.group === 'biome') d.r = biome_radius;
        ...
        d.r = degree_scale(d.degree);
      } else if (d.type === 'region') d.r = region_radius;
      else d.r = radius_scale(d.type);

      if (d.type === 'concept' && d.group === 'biome')
        d.fill = scale_color_biome(d.id);
  ...
  function createPrimaryData() {
    ...
    nodes_primary = nodes.filter(
      d => d.type === 'element' || d.group === 'biome'
    );
  ...
  function setupPrimaryPlacement() {
    ...
    nodes_primary.forEach(d => {
      ...
      let angle =
        d.type === 'element'
          ? biome_by_id[d.biome_main].angle
          : biome_by_id[d.id].angle;
  ...
  function testTypeId(d, type) {
    return (
      node_by_id[d.source.id].type === type ||
      node_by_id[d.target.id].type === type
    );
  }
  ...
  function runPrimarySimulation() {
    ...
    simulation_primary.nodes(nodes_primary).stop();
    //     .on("tick", drawQuick)
    //     .on("end", () => {
    ...
    for (let i = 0; i < n_ticks; ++i) {
      ...
      simulation_primary
        .force('collide')
        .strength(Math.pow(i / n_ticks, 2) * 0.7);
    } //for i
    // //Show the end result
    // drawQuick()
  ...
  function runSecondarySimulation() {
    ...
    simulation_secondary.nodes(nodes).stop();
    // .on("tick", drawQuick)
    // .on("end", () => {
    //     //Save the final positions for zooming and panning
    //     nodes.forEach(d => {
  ...
  function zoomChart() {
    ...
    nodes.forEach(d => {
      ...
      if (d.type !== 'element')
        d.stroke_width = d.stroke_width_fixed * transform.k;

      //https://stackoverflow.com/questions/23759457
      //Find a first guess for the font-size
      let font_size = roundTo(
        ((d.r - d.stroke_width) / 2.5) * (10 / (d.label.length + 1)),
        2
      );
      if (d.type === 'concept' && d.r > 10 && font_size > 6) {
        // let old = font_size
        let font_width =
          (d.r - d.stroke_width) * 2 * (d.label.length >= 10 ? 0.7 : 0.75);
  ...
  function calculateZoomLevel(node) {
    ...
    return Math.min(
      max_zoom,
      Math.min(width * 0.4, height * 0.4) / (2 * node.r_fixed)
    );
  ...
  function setMultiSelection(node, connected_node_by_id, connected_edge_by_id) {
    ...
    let edges_connected = edges.filter(
    ...
    let nodes_connected = nodes.filter(
      d => neighboring(node, d) || node.id === d.id
    );
  ...
  function mouseClickChart() {
    ...
    if (click_double) {
      //Double click
      if (found) {
        //Zoom in and set a click
        d3.event.preventDefault();
        //If the person double clicks on the same again, zoom out
        if (found === current_double_click)
          performManualZoom(found, start_zoom);
      ...
      if (click_active && found_edge) {
        ...
        let node =
          current_click.id === found_edge.source.id
            ? found_edge.target
            : found_edge.source;
  ...
  function drawHiddenEdges(node) {
    ...
    edges_selected.forEach(d => {
      drawEdges(ctx_hidden, d, d.stroke_hidden, Math.max(10, getLineWidth(d)));
    });
  ...
  function mouseMoveActions(found_hover, found_edge, m) {
    ...
    if (current_hover === found_hover && found_hover !== null) {
      ...
      let other_side =
        found_edge.source.id === current_click.id
          ? found_edge.target
          : found_edge.source;
  ...
  function setSelection(node) {
    ...
    if (!click_active && node.type === 'region') {
      ...
      edges_selected = edges.filter(
        d => d.source === node || d.target === node
      );
      //Save only those that are neighbors
      nodes_selected = nodes.filter(
        d => neighboring(node, d) || node.id === d.id
      );
    } //else
    ...
    if (!click_active && (node.type === 'region' || node.type === 'country')) {
      ...
      nodes_selected
        ...
        .forEach(d => {
          ...
          d.biomes.forEach(b => {
            biomes_selected.push(b);
          });
        }); //forEach
      //Get the unique list
      biomes_selected = [...new Set(biomes_selected)];
      //Add the biomes to the selected nodes
      biomes_selected.forEach(d => {
        nodes_selected.push(biome_by_id[d]);
      });
    } //if
  ...
  function connectedNodes(n) {
    ...
    let connected_edges = edges.filter(
      d =>
        d.target === n &&
        (d.source.type === 'region' ||
          d.source.type === 'country' ||
          d.source.type === 'element')
    );
    ...
    let connected_nodes = nodes.filter(
      d =>
        edge_by_id[d.id + ',' + n.id] &&
        (d.type === 'region' || d.type === 'country' || d.type === 'element')
    );
    //Save in array for drawing
  ...
  function mouseOutNode() {
    ...
    if (forced_load) {
      ...
      clearCanvas([ctx_edges, ctx_nodes, ctx_hover, ctx_hidden]);
      let zoom_line_width =
        zoom_edge_increase(transform.k) * primary_line_width;
      edges_primary.forEach(d => {
  ...
  function getLineWidth(d) {
    ...
    let line_width =
      ...
      Math.max(
        min_edge,
        zoom_edge_increase(transform.k) *
          (d.focus === 'primary' ? primary_line_width : secondary_line_width)
      );
  ...
  function drawNodes(ctx, d, r, opacity) {
    ...
    if (
      ...
      d.img &&
      d.img_loaded &&
      (r > photo_size || show_photos)
    ) {
      //When saving, making the stroke width smaller
      let radius = show_photos
        ? r - (d.stroke_width * 0.2) / 2
        : r - d.stroke_width / 2;
    ...
    if (d.type === 'whc') {
      ...
      if (d.img && d.img_loaded && (r > photo_size || show_photos)) {
        ...
        if (d.biome_main) {
          ...
          //Stroke the pie chart, but don't make the strokes too thick
          if (d.stroke_width / transform.k < 4)
  ...
  function calculateEdgeCenters(edges) {
    edges.forEach(d => {
      //Find a good radius
      d.r =
        Math.sqrt(sq(d.target.x - d.source.x) + sq(d.target.y - d.source.y)) *
        2;
  ...
  function calculateEdgeGradient(edges) {
    ...
    function createGradient(d, variable, alpha) {
      ...
      if (d.source.x && d.target.x) {
        d[variable] = ctx_edges.createLinearGradient(
          d.source.x,
          d.source.y,
          d.target.x,
          d.target.y
  ...
  function renderNodeLabels(ctx, nodes_to_label, opacity) {
    ...
    let nodes_big = nodes_to_label.filter(
      d =>
        d.type === 'concept' &&
        (d.font_size > 7 || (do_print_run && d.font_size > 3 / sf_scale))
    );
  ...
  function renderClickIcon(node) {
    ...
    modal_icon_cross.attr(
      'd',
      d3
        .symbol()
        .size(Math.pow(modal_cross_size, 2) * 8)
        .type(d3.symbolCross)
    );
  ...
  chart.saveImage = (width_print = 20, units = 'cm', photos = false) => {
    ...
    if (sf_new * width * sf_new * height > 268435456)
      console.log(
        'requested canvas is probably too big for the browser to handle'
      );
    ...
    function createPrintCanvas() {
      ...
      try {
        ...
        console.log(
          "Unable to automatically download the file. Instead an image has been added to the screen all the way at the bottom. Right-click and save this image. If no 'Save-as' window pops up, a browser extension might be blocking it. In that case try to open the page in a Chrome Incognito Window, or deactivate all extension, and redo the process."
        );
      } //try-catch
