...
function createConstellationVisual() {
  ...
  const node_community_color = d3.scaleLinear().range([0, 1]);
  const dark_rainbow = [
    '#EFB605',
    '#E58903',
    '#E01A25',
  ...
  const node_color_interpolate = d3
    .scaleLinear()
    .domain(
      d3.range(dark_rainbow.length).map(d => d / (dark_rainbow.length - 1))
    )
    .range(dark_rainbow)
  ...
  const node_charge_degree_scale = d3
    .scaleLinear()
    .domain([10, 250])
    .range([-10, -1000]);
    ...
    simulation_primary = d3
      ...
      .force(
        ...
        d3
          .forceManyBody()
          .strength(d =>
            d.type === 'element' ? -10 : node_charge_degree_scale(d.degree)
          )
  ...
  chart.createChart = function(callback) {
    ...
    edges_primary = edges.filter(
      d => d.weight === primary_edge_weight || testTypeId(d, 'whc')
    );
  ...
  function filterNodeData() {
    ...
    nodes = nodes.filter(d => {
      if (d.type === 'concept') {
        if (d.group === 'main' || d.group === 'unesco' || d.group === 'nature')
          return true;
        else return false;
      } else return true;
    }); //filter
  ...
  function filterUnconnectedNodes() {
    ...
    let connected_ids = nodes
      ...
      .map(d =>
        linked_to_id[d.id].filter(n => n.type === 'country').map(n => n.id)
      );
  ...
  function createPrimaryData() {
    ...
    edges_primary = edges.filter(
      d => d.weight === primary_edge_weight || testType(d, 'whc')
    );
  ...
  function testType(d, type) {
    return (
      node_by_id[d.source].type === type || node_by_id[d.target].type === type
    );
  }
  ...
  function findCommunityCenters() {
    ...
    let community = jLouvain()
      .nodes(nodes_primary.map(d => d.id))
      .edges(edges_primary);
    ...
    community_count = community_count.sort((a, b) => {
      return d3.ascending(a.item, b.item);
    });

    //Find the id of the biggest group
    let counts = community_count.map(d => d.frequency);
    let community_biggest =
      community_count[counts.indexOf(Math.max(...counts))].item;
    ...
    community_count = replaceSmallCommunities(
      community_count,
      community_connections,
      community_count_by_id,
      community_biggest
    );

    //Which concepts are the largest per group
    community_count.forEach(d => {
      let chosen = nodes_primary.filter(
        n => n.community === d.item && n.type === 'concept'
      );
    ...
    community_count.forEach((d, i) => {
      d.angle_slice = Math.round(
        Math.pow(
          Math.max(num / (community_count.length * 4), d.frequency),
          0.75
        )
      );
  ...
  function calculateCommunityConnections() {
    ...
    edges.forEach(d => {
      ...
      if (typeof sc === 'undefined' || typeof tc === 'undefined' || sc === tc)
        return;
  ...
  function replaceSmallCommunities(
    ...
    community_count
      ...
      .forEach(d => {
        let connections = community_connections.filter(
          c => c.community_a === d.item
        );
  ...
  function drawCommunityCenters() {
    ...
    community_count.forEach(d => {
      ctx_nodes.fillStyle = node_color_interpolate(
        node_community_color(d.item)
      );
      ctx_nodes.beginPath();
  ...
  function runSecondarySimulation() {
    ...
    for (let i = 0; i < n_ticks; ++i) {
      ...
      //Ramp up collision strength to provide smooth transition
      collide.strength(
        Math.min(2.2, 0.3 + Math.pow(i / (n_ticks * 0.6), 2) * 1)
      );
  ...
  function setSelection(node) {
    ...
    if (node.type !== 'region' || click_active) {
      ...
      //For regions look all the way down to the countries and elements
      edges_selected = [];
      nodes_selected = [node];
  ...
  function fadeCanvasBackIn() {
    ...
    edges_primary.forEach(l => {
      l.interpolate_opacity = d3.interpolate(0, 1);
    });
